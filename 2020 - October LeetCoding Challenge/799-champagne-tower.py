# Question: https://leetcode.com/explore/challenge/card/october-leetcoding-challenge/562/week-4-october-22nd-october-28th/3508/
...
class Solution:
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        ...
        for i in range(query_row):
            level_next = [0] * (len(level)+1)
            
            for j in range(len(level)):
                amount = (level[j] - 1)/2
                if amount > 0:
                    level_next[j] += amount
                    level_next[j+1] += amount
